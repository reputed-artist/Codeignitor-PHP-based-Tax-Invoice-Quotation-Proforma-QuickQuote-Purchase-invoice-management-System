...
{
    protected $table = 'delivery_addresses'; // Specify your table name
    protected $primaryKey = 'delid'; // Specify your primary key column

    

    protected $allowedFields = ['invid', 'name','address','mob']; // Specify allowed fields
...
public function getdeliverydata($edit_id)
{
    // Use query builder methods properly
    return $this->db->table('delivery_addresses')  // Assuming your table is 'table_name'
                    ->where('invid', $edit_id)
                    ->get()
                    ->getRowArray();
}
