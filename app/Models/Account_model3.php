...
class Account_model3 extends Model
{
    ...
    public function get_last_cid()
{
    ...
    /**
     * Fetch all account details along with the current closing balance.
     *
     * Closing balance = opening balance + credits - debits, using the same
     * debit/credit conventions as getLedgerByFY():
     *
     *   u_type 0 (Client)   : credit = sales (invtest2)             => closing = opening + sales - payments
     *   u_type 1 (Supplier) : credit = purchases (purchaseinv2)     => closing = opening + purchases - payments
     *   u_type 2 (Dual)     : credit = purchases - sales, debit = payments
     *                                                               => closing = opening + purchases - sales - payments
     */
    public function getAccountDetailsWithClosingBalance()
    {
        $sql = "SELECT 
            a.aid,
            c.cid,
            c.c_name,
            SUBSTRING_INDEX(c.c_add, ',', -1) AS location,
            c.mob,
            c.u_type,
            a.opening_bal,
            a.created,
            a.is_hidden,
            ROUND(
                COALESCE(a.opening_bal, 0)
                + COALESCE(
                    CASE c.u_type
                        WHEN 0 THEN (SELECT COALESCE(SUM(it.totalamount), 0) FROM invtest2 it WHERE it.cid = c.cid)
                        WHEN 1 THEN (SELECT COALESCE(SUM(pin.totalamount), 0) FROM purchaseinv2 pin WHERE pin.cid = c.cid)
                        WHEN 2 THEN (SELECT COALESCE(SUM(pin.totalamount), 0) FROM purchaseinv2 pin WHERE pin.cid = c.cid)
                                 - (SELECT COALESCE(SUM(it.totalamount), 0) FROM invtest2 it WHERE it.cid = c.cid)
                        ELSE 0
                    END,
                    0
                )
                - COALESCE((SELECT SUM(ph.amount) FROM paidhistory ph WHERE ph.cid = c.cid), 0),
                2
            ) AS closing_bal
        FROM account a
        JOIN client c ON c.cid = a.cid
        ORDER BY closing_bal DESC, a.aid DESC";
