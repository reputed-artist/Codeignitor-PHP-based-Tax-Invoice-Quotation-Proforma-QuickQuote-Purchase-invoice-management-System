...
if (hasError) {
    return;
        ...
        $.ajax({
            type: "POST",
            url: base_url + "/purchaseinv/insert",
            data: formData,
            dataType: 'json', 
            //contentType: false,           
            success: function(response) {
                ...
                if (response.success) {
                    ...
                    Swal.fire({
                    title: "Good!",
                    text: "Purchase Invoice Data Inserted!",
                    icon: "success",
                    showConfirmButton: false, // Hide the OK button
                   timer: 3000, // Close the popup after 3 seconds (3000 milliseconds)
                  }).then(function() {

                 window.location.href = base_url + '/purchaseinv/genpurchaseinv'
...
// $(document).on("click", "#edit_purinv", function() {
//     var edit_id = $(this).data("id");

//     console.log(edit_id);

//     $.ajax({
//         url: base_url + "/purchaseinv/editpurchaseinv/"+edit_id,
//         type: "get",
//         dataType: "JSON",
//         data: { orderid: edit_id }, // Pass the order ID
//         success: function(response) {
//             console.log("AJAX response:", response); // Debug the response

//             // Iterate over each item in the records
//             for (var i = 0; i < response.records.length; i++) {

//                 console.log(response.records[i]);
//                 // Get the dynamic row number (start from 1, 2, 3, etc.)
//                 var rowNum = i + 1;

//                 // Set the values for the fields dynamically
//                 $("#productName_" + rowNum).val(response.records[i].item_name);
//                 $("#hsn_" + rowNum).val(response.records[i].hsn);
//                 $("#quantity_" + rowNum).val(response.records[i].quantity);
//                 $("#price_" + rowNum).val(response.records[i].price);
//                 $("#total_" + rowNum).val(response.records[i].total);
//                 $("#productDesc_" + rowNum).val(response.records[i].item_desc || ''); // Fallback if description is null
//             }

//           if (response.records2.length > 0) {
//                 var record2 = response.records2[0]; // Assume only one invoice record for this order
//                 $("#invid").val(record2.invid);
//                 $("#supplier").val(record2.cid).trigger("change"); // Set the supplier dropdown
//                 $("#taxamount").val(record2.taxamount);
//                 $("#totalamount").val(record2.totalamount);
//             }
//         },
//         error: function(xhr, status, error) {
