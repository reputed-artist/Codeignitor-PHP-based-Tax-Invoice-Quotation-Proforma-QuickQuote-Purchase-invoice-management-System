...
$(document).on("click", "#viewsupplier", function(e) {
    ...
    console.log(infoid);
    window.location.href = `viewsupplierinfo/${infoid}`;
});
...
function fetch() {
    ...
    $.ajax({
        type: "get",
        url: base_url + "/supplier/managesupplier", // Using base_url defined in the view
        //'type': 'GET',
        dataSrc: 'aaData',
        dataType:'json',
        success: function(response) {
        ...
        // fixedColumns: true, 
                ...
                'columns': [
                    ...
                    { 'data': 'c_type',render: function (data, type, row, meta) {
                      ...
                      { 'data': 'u_type',render: function (data, type, row, meta) {
                                //console.log(data);
                                 if(data == "0")
                                {
                                  return '<span class="label bg-primary">' + 'Client' + '</span>';
                                }

                                else if(data == "1")
                                {
                                  return '<span class="label bg-navy">' + 'Supplier' + '</span>';
                                }
                                else if(data == "2"){
                                    return '<span class="label bg-purple">' + 'Dual (Cust/Sup)' + '</span>';
                                }
...

                    {
                      'data': 'viewaction',
                         render: function (data, type, row, meta) {
                                return `<a href="viewsupplierinfo/${row.cid}" class="btn btn-warning btn-xs" id="viewsupplier" data-id="${row.cid}">
...
$(document).on("click", "#submit", function(e) {
    ...
    $('#c_name, #c_add, #phone, #gst, #ctype, #utype').removeClass('is-invalid');
...
if ($('#utype').val().trim() === '') {
    ...
    var u_type = $("#utype").val();
      ...
      //var u_type = 1; // Assuming you want this value
var fd = new FormData();
fd.append("cid", cid);
                ...
                Swal.fire({
                  ...
                  $("#example1").DataTable().clear().destroy();
                    fetch();
                     ...
                     $.ajax({
                    url:  base_url +'/supplier/get_next_id', // URL to get the next ID
                    method: 'GET',
...
$(document).on("click", "#delete_product", function(e) {
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            $.ajax({
                type: "get",
                url: base_url+"/supplier/delete/" + parseInt(del_id),
...
$(document).on("click", "#Update", function(e) {
    ...
    $('#c_nameedit, #c_addedit,#phoneedit, #gstedit, #ctypeedit, #utype1').removeClass('is-invalid');
...
if ($('#utype1').val().trim() === '') {
    ...
    //console.log("utype1 value:", $("#utype1").val()); // Should log a non-null value
