...
function getExportButtons2(tableId, exportColumns) {
    return [
        {
            extend: 'copyHtml5',
...
// },
     
  {
    ...
    footer: true,
    title: function() {
        var companyName = $("#Company").text() || "Company";
        var clientName = $("#Location").text() || "Client";
    ...
    customize: function(xlsx) {
        try {
            ...
            // Move all rows down by 5 rows (for headers)
            $rows.each(function() {
                var oldRowNum = parseInt($(this).attr('r'));
                if (!isNaN(oldRowNum)) {
                    $(this).attr('r', oldRowNum + 5);
                    $(this).find('c').each(function() {
                        var oldRef = $(this).attr('r');
                        var colLetter = oldRef.replace(/\d+/g, '');
                        var newRef = colLetter + (oldRowNum + 5);
                        $(this).attr('r', newRef);
                    });
                }
            });
            ...
            // Add header rows
            var headers = 
                '<row r="1"><c r="D1" t="inlineStr"><is><t>' + companyName + '</t></is></c></row>' +
                '<row r="2"><c r="D2" t="inlineStr"><is><t>' + clientName + '</t></is></c></row>' +
                '<row r="3"><c r="D3" t="inlineStr"><is><t>Financial Year: ' + fy + '</t></is></c></row>' +
                '<row r="4"><c r="F4" t="inlineStr"><is><t>Opening Balance: ' + openingBalance + '</t></is></c></row>' +
                '<row r="5"><c r="A5" t="inlineStr"><is><t>Generated on: ' + new Date().toLocaleDateString() + '</t></is></c></row>';
            
            var firstRow = sheetData.find('row').first();
            if (firstRow.length) {
                firstRow.before(headers);
            } else {
                sheetData.append(headers);
            }
            
            // ========== NOW RUN YOUR ORIGINAL FOOTER CODE ==========
            var $rows = $sheet.find('row');
            console.log('Total rows in sheet:', $rows.length);
            ...
            // Get the last row number (after headers added)
            var lastRow = $rows.last();
            var lastRowNum = parseInt(lastRow.attr('r')) || $rows.length;
            
            // Your original closing balance code
            var closingRow = '<row r="' + (lastRowNum + 1) + '">' +
                '<c r="D' + (lastRowNum + 1) + '" t="inlineStr"><is><t>Closing Balance</t></is></c>' +
                '<c r="F' + (lastRowNum + 1) + '" t="n"><v>' + (parseFloat(closingBalance.replace(/,/g, '')) || 0) + '</v></c>' +
                '</row>';
            
            // Insert before the closing </sheetData>
            sheetData.append(closingRow);
            
            // Also add Total Credit & Debit row
            var totalCredit = $("#totalcreditamt").text() || "0.00";
            var totalDebit = $("#totaldebitamt").text() || "0.00";
            
            var totalRow = '<row r="' + (lastRowNum + 2) + '">' +
                '<c r="D' + (lastRowNum + 2) + '" t="inlineStr"><is><t>Total Bal. Credit & Debit</t></is></c>' +
                '<c r="E' + (lastRowNum + 2) + '" t="n"><v>' + parseFloat(totalCredit.replace(/,/g, '')) + '</v></c>' +
                '<c r="F' + (lastRowNum + 2) + '" t="n"><v>' + parseFloat(totalDebit.replace(/,/g, '')) + '</v></c>' +
                '</row>';
            
            sheetData.append(totalRow);
            
            // Update dimensions
            $sheet.find('dimension').attr('ref', 'A1:G' + (lastRowNum + 2));
            
        } catch (error) {
            console.error('Error in Excel customization:', error);
        }
    },
    filename: function() {
        ...
        return companyName.replace(/[^a-zA-Z0-9]/g, '_') + '_Ledger_FY_' + fy + '_' + date + '.xlsx';
    }
},
...
//         var tableElement = dt.table().node();
//         var $table = $(tableElement);
        
//         // Store original thead
//         var originalThead = $table.find('thead').clone();
//         var originalTheadHtml = $table.find('thead').html();
        
//         // Create new thead with title rows
//         var newThead = $('<thead></thead>');
        
//         // Company row - spanning all columns
//         var companyRow = $('<tr></tr>');
//         var companyCell = $('<th colspan="7" style="text-align: center; font-size: 18px; font-weight: bold; padding: 12px; background-color: #f8f9fa; border: 1px solid #ddd;"></th>');
//         companyCell.text(companyName);
//         companyRow.append(companyCell);
//         newThead.append(companyRow);
        
//         // Location & FY row
//         var locationRow = $('<tr></tr>');
//         var locationCell = $('<th colspan="7" style="text-align: center; font-size: 15px; font-weight: bold; padding: 10px; background-color: #f8f9fa; border: 1px solid #ddd;"></th>');
//         locationCell.html( clientName + ' &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; Ledger FY: ' + fy);
//         locationRow.append(locationCell);
//         newThead.append(locationRow);
        
//         // Empty spacing row
//         var spaceRow = $('<tr></tr>');
//         var spaceCell = $('<th colspan="7" style="padding: 5px; border: 1px solid #ddd;"></th>');
//         spaceCell.html('&nbsp;');
//         spaceRow.append(spaceCell);
//         newThead.append(spaceRow);
...
// },
{
    ...
    action: function (e, dt, node, config) {
        ...
        // Row 1: Company name - spanning all columns
        var companyRow = $('<tr></tr>');
        var companyCell = $('<th colspan="7" style="text-align: center; font-size: 18px; font-weight: bold; padding: 12px; background-color: #f8f9fa; border: 1px solid #ddd;"></th>');
        companyCell.text(companyName);
        companyRow.append(companyCell);
        ...
        // Row 2: Location and FY with proper spacing
        var locationRow = $('<tr></tr>');
        var locationCell = $('<th colspan="7" style="text-align: center; font-size: 15px; font-weight: bold; padding: 10px; background-color: #f8f9fa; border: 1px solid #ddd;"></th>');
        locationCell.html('Location : ' + clientName + ' &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; Ledger FY: ' + fy);
        locationRow.append(locationCell);
        newThead.append(locationRow);
